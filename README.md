...
Выкат на прод-хост — одной командой: `./scripts/deploy.sh` (её же вызывает
автодеплой, поэтому ручной и автоматический выкат не расходятся). Фронт там
обновляется не так, как остальное, — подробности в шапке скрипта.
