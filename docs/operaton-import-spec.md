...
Правила:
* `grid_columns` формы = 2 по умолчанию; если в схеме нет ни одного `columns` —
  оставляем 2 и раскладываем поля по одному в строку;
...
1. **Множественный выбор** — подтвердить решение §5.1 (новый тип `checkbox_group`
   против автосоздания справочников).
2. **BPMN-импорт (Фаза 2)** — нужен ли вообще, или в вашем контуре формы всегда
   лежат отдельными `.form`-файлами?
...
4. **Ключи полей**: допустимо ли переименование `applicant.firstName` →
   `applicant_firstName`, или требуется поддержка точек в `field.id` (тогда нужна
   правка движка формул и зависимостей)?
5. Какая версия Operaton / `schemaVersion` целевая — от неё зависит набор
   компонентов, которые реально встретятся.
