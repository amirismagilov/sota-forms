...
@router.get("/theme")
async def get_theme(db: AsyncSession = Depends(get_db), aid: str = Depends(require_account)):
    acc = await get_account_by_id(db, aid)
    return {"design_tokens": acc.design_tokens, "webhook_default": acc.webhook_default}


@router.put("/theme")
async def update_theme(body: ThemeIn, db: AsyncSession = Depends(get_db), aid: str = Depends(require_account)):
    acc = await get_account_by_id(db, aid)
